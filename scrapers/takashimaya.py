...
STORE = "新宿高島屋"
BASE_HOST = "https://www.takashimaya.co.jp"
...
def scrape() -> list[Event]:
    ...
    try:
        with sync_playwright() as p:
            ...
            page = ctx.new_page()
            page.goto(EVENT_URL, wait_until="domcontentloaded", timeout=50000)
            page.wait_for_timeout(4000)

            html = page.content()
            print(f"[takashimaya] page size: {len(html)} chars", flush=True)

            # 記事カードを汎用的に取得
            items = page.query_selector_all(
                "li.topicsList__item, .topics-list-item, article.topics, "
                ".eventList li, ul.topicsList > li, .topicsListArea li"
            )
            print(f"[takashimaya] items found: {len(items)}", flush=True)

            for item in items:
                try:
                    title_el = item.query_selector("h2, h3, h4, .topicsList__title, .title")
                    if not title_el:
                        continue
        ...
        print(f"[takashimaya] scrape error: {e}", flush=True)
